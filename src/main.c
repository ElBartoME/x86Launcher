...
int main() {
	...
	/* ************************************** */
	gamedata = (gamedata_t *) calloc(1,sizeof(gamedata_t));
	gamedata->next = NULL;

	/* ************************************** */
	/* Parse the gamedirs that are set */
	/* ************************************** */
	gamedir = (gamedir_t *) calloc(1,sizeof(gamedir_t));
	gamedir->next = NULL;
	...
	// Screenshot buffer
	screenshot_bmp = (bmpdata_t *) calloc(1,sizeof(bmpdata_t));
	screenshot_bmp->pixels = NULL;

	// Screenshot state line buffer
	screenshot_bmp_state = (bmpstate_t *) calloc(1, sizeof(bmpstate_t));
	...
	// Save the true head of the game list so getGameid() can always search from the start
	...
	// Show artwork for initially selected game
	if (state->has_images) {
	...
	//
	// ======================
	// Always ensure gamedata points to the true head before entering the loop
	gamedata = gamedata_head;
	if (config->verbose) {
			...
			switch (user_input) {
				...
				case (input_up):
					...
					if (state->selected_line == 0) {
						...
						// Land on the last entry of the new page
						int page_start = (state->selected_page - 1) * ui_browser_max_lines;
						int page_entries = state->selected_max - page_start;
						if (page_entries > ui_browser_max_lines) page_entries = ui_browser_max_lines;
						state->selected_line = page_entries - 1;
				...
				case (input_down):
					...
					// Down current list by one row
					{
						...
						int page_entries = state->selected_max - page_start;
						if (page_entries > ui_browser_max_lines) page_entries = ui_browser_max_lines;
						if (state->selected_line == page_entries - 1) {
