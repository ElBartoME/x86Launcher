...
// Store these as globals instead of lfb directly
...
int gfx_Init() {
	...
	vbeinfo = (vbeinfo_t *) calloc(1,sizeof(vbeinfo_t));
	vesamodeinfo = (vesamodeinfo_t *) calloc(1,sizeof(vesamodeinfo_t));
	...
	vesamodeinfo_t *testmode = calloc(1,sizeof(vesamodeinfo_t));
	int r = vesa_GetModeInfo(0x100, testmode);
	printf("GetModeInfo(0x100) = %d, attrs = 0x%x, res = %dx%d %dbpp\n",
		   r, testmode->ModeAttributes, testmode->XResolution, testmode->YResolution, testmode->BitsPerPixel);
	free(testmode);
	...
	status = vesa_SetMode(GFX_VESA_DESIRED_LFB);
	if (status < 0) {
		if (GFX_VERBOSE) {
	...
	lfb_meminfo.address = vesamodeinfo->PhysBasePtr;
	lfb_meminfo.size = (GFX_COLS * GFX_ROWS + 4095) & ~4095;
	if (__dpmi_physical_address_mapping(&lfb_meminfo) != 0) {
		printf("ERROR: DPMI mapping failed!\n"); fflush(stdout);
		return -1;
	}
	lfb_phys_address = lfb_meminfo.address;
...
void gfx_Clear() {
	...
	memset(vram_buffer, PALETTE_UI_BLACK, VRAM_END);
}
...
void gfx_Flip() {
    __djgpp_nearptr_enable();
    unsigned char *lfb_ptr = (unsigned char *)(lfb_phys_address + __djgpp_conventional_base);
    memcpy(lfb_ptr, vram_buffer, VRAM_END);
}
...
int gfx_BitmapAsync(int x, int y, bmpdata_t *bmpdata, FILE *bmpfile, bmpstate_t *bmpstate, int remap_palette, int reserved_palette) {
	...
	int i;              // Loop counter
	unsigned char *ptr; // Access pixels in pixel buffer
	...
	// ADD THIS CHECK
	if (new_y >= GFX_ROWS || new_y < 0) {
		bmpstate->rows_remaining--;
		if (bmpstate->rows_remaining < 1) bmpstate->rows_remaining = 0;
		return 0;
	}
...
int gfx_BitmapAsyncFull(int x, int y, bmpdata_t *bmpdata, FILE *bmpfile, bmpstate_t *bmpstate, int remap_palette, int reserved_palette) {
	...
	if (status != 0) {
		...
		// Set palette ONCE before the loop
		if (reserved_palette == 0) {
			pal_BMP2Palette(bmpdata, 0);  // load artwork palette into free slots
		}
